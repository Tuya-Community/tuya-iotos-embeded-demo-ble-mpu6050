...
//    TUYA_APP_LOG_INFO("callback: %02x", event->evt);
    ...
    switch(event->evt) {
        ...
        case TUYA_BLE_CB_EVT_DP_DATA_RECEIVED: {
            tuya_ble_dp_data_send(g_sn++, DP_SEND_TYPE_ACTIVE, DP_SEND_FOR_CLOUD_PANEL, DP_SEND_WITHOUT_RESPONSE, 
                                    event->dp_received_data.p_data, event->dp_received_data.data_len);
#if TUYA_BLE_SDK_TEST
            if(event->dp_received_data.data_len < 200) {
                ...
                tuya_ble_inverted_array((void*)&dp_received_data_len, sizeof(uint16_t));
                tuya_ble_sdk_test_send(TY_UARTV_CMD_DP_WRITE, (void*)&dp_received_data_len, sizeof(uint16_t));
            }
#endif
        } break;
